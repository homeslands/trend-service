...
export class ProductService {
  ...
  async uploadProductImage(
    ...
    product.image = await this.fileService.uploadFile(file);
    const updatedProduct = await this.productRepository.save(product);

    this.logger.log(
      `Product image ${product.image} uploaded successfully`,
      context,
    );
  ...
  async getAllProductsPagination(
    ...
    if (_.isNil(query.isHidden)) {
      findOptionsWhere.isHidden = false;
    } else {
      findOptionsWhere.isHidden = query.isHidden;
    }
    ...
    if (query.voucherCampaignTemplate) {
      const template = await this.voucherCampaignTemplateRepository.findOne({
        where: { slug: query.voucherCampaignTemplate },
      });
      const productSlugs = template?.productSlugs ?? [];
      findOptionsWhere.slug = query.isAppliedVoucherCampaignTemplate
        ? In(productSlugs)
        : Not(In(productSlugs));
    }
  ...
  async deleteProduct(slug: string): Promise<number> {
    ...
    const product = await this.productRepository.findOne({
      ...
      relations: {
        ...
        productChefAreas: true,
        productAnalyses: true,
      },
    });
    if (!product)
      throw new ProductException(ProductValidation.PRODUCT_NOT_FOUND);
    ...
    const templateMenuItems = product.menuItems.filter(
      (item) => item.menu?.isTemplate,
    );
    ...
    await this.transactionService.execute<void>(
      async (manager) => {
        ...
        await manager.remove(templateMenuItems);
        await manager.remove(product.productChefAreas);
      },
      () => {
        this.logger.log(`Product ${slug} hidden successfully`, context);
      },
      (error) => {
        this.logger.error(`Error hiding product: ${error.message}`, context);
        throw new ProductException(ProductValidation.PRODUCT_HIDDEN_FAILED);
      },
    );

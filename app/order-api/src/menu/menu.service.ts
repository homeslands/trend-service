...
export class MenuService {
   ...
   * @returns {Promise<AppPaginatedResponseDto<MenuResponseDto>>} The specific menu was retrieved with paginated menu items
   * @throws {MenuException} Menu not found
   */
  async getMenuPublic(
    ...
    if (_.isBoolean(query.isTopSell)) {
      if (query.isTopSell) {
        menu.menuItems = menu.menuItems.filter(
          (item) => item.product.isTopSell,
        );
      } else {
        menu.menuItems = menu.menuItems.filter(
          (item) => !item.product.isTopSell,
    ...
    menu.menuItems = menu.menuItems.filter(
      (item) =>
        item.product.isGift === false && item.product.isCustomPrice === false,
    );

    // Paginate menu items after all in-memory filters
    ...
    if (query.hasPaging) {
      menu.menuItems = menu.menuItems.slice(
        (page - 1) * pageSize,
        page * pageSize,
      );
    }
    ...
    return {
      ...
      items: [this.mapper.map(menu, Menu, MenuResponseDto)],
      total,
      page,
      pageSize,
      totalPages,

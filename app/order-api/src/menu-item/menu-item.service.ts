...
export class MenuItemService {
  ...
  /**
   * Retrieve all menu items (staff, manager, admin, super admin can access)
   * @returns {Promise<AppPaginatedResponseDto<MenuItemResponseDto>>} The paginated menu items data
   */
  async findAll(
    query: GetMenuItemQueryDto,
    currentUserDto: CurrentUserDto,
  ): Promise<AppPaginatedResponseDto<MenuItemResponseDto>> {
    ...
    return this.findAllMenuItems(query, hideRestrictedProducts);
  }

  /**
   * Retrieve all menu items public
   * @returns {Promise<AppPaginatedResponseDto<MenuItemResponseDto>>} The paginated menu items data
   */
  async findAllPublic(
    query: GetMenuItemQueryDto,
  ): Promise<AppPaginatedResponseDto<MenuItemResponseDto>> {
    return this.findAllMenuItems(query, true);
  }

  private async findAllMenuItems(
    query: GetMenuItemQueryDto,
    hideRestrictedProducts: boolean,
  ): Promise<AppPaginatedResponseDto<MenuItemResponseDto>> {
    const context = `${MenuItemService.name}.${this.findAllMenuItems.name}`;
    ...
    if (hideRestrictedProducts) {
      findOptionsWhere.product = {
        ...(findOptionsWhere.product as FindOptionsWhere<Product>),
        isGift: false,
        isCustomPrice: false,
      };
    }
    ...
    const [menuItems, total] = await this.menuItemRepository.findAndCount({
      ...
      skip: query.hasPaging ? (query.page - 1) * query.size : undefined,
      take: query.hasPaging ? query.size : undefined,
    });
    ...
    return {
      ...
      items: this.mapper.mapArray(menuItems, MenuItem, MenuItemResponseDto),
      total,
      page,
      pageSize,
      totalPages,
    } as AppPaginatedResponseDto<MenuItemResponseDto>;
  }
